...
int main(int argc, char** argv)
{
  ...
  if (part && G4String(part) == "1") {  //这里没有使用这个环境变量
    G4cout<<"++++++++++++++++++++++++++++"<<G4endl;
    bPartial = TRUE;
  }else
  {
  ...
  if (!bPartial) {
    ...
    if (nest && G4String(nest) == "1") {
      //测试输出
      G4cout << "1111111111" << G4endl;
      theGeometry = new DicomNestedParamDetectorConstruction();//这一项更好
    }
    else {
      G4cout << "2222222222" << G4endl;
      theGeometry = new DicomRegularDetectorConstruction(); //用的这一项
    }
  }
  else {
  ...
  new DicomIntersectVolume();//在不改变患者体模（phantom）的位置的前提下，创建一个旋转/平移后的用户体积（如圆柱、盒子），判断 phantom 中哪些体素完全包含在这个体积内，然后输出一个只保留这些体素的新 phantom 文件。
