...
/*
检测格式 → 读取头信息（Tag）→ 判断编码方式 → 识别像素数据起点 → 提取结构参数 → 构造切片 → 读取体素数据 → 存入 phantom
成员变量
含义说明
fImplicitEndian
是否为隐式VR格式（没有”DICM”标签）
fLittleEndian
是否为小端序（默认是）
fPixelSpacingX/Y
DICOM中PixelSpacing字段提取
fRows、fColumns
像素行列数
fSliceLocation
当前切片的位置（Z轴）
...
{
  std::ifstream checkData(fDriverFile.c_str());// Data.dat
  char* oneLine = new char[128];

  //打印一下fDriverFile.c_str()
  //G4cout << "-=-=-=-=fDriverFile.c_str() = " << fDriverFile.c_str() << G4endl;
